...
using namespace RooFit ;
    ...
    std::vector<std::string> histpaths = {
        "path/in/file/to/histogram1",
        "path/in/file/to/histogram2",
    };
    ...
    // Matrix definition
    const Int_t MatrixDim = 10;
    TH2F* h2 = new TH2F("Matrix","", 2,-2.5,2.5,2,-2.5,2.5);
    ...
    for (auto& filepath : filepaths) {
        file = TFile::Open(filepath.c_str());
         ...
         for (auto& histpath : histpaths) {
            // Get the histogram.
            inputHistogram = dynamic_cast<TH1F*>(inputFile->Get(histpath.c_str()));

            // Check if we got the histogram.
            if (!inputHistogram) {
                std::cout << "Error getting histogram: " << histpath << " from file: " << filepath << std::endl;
                continue;
            }

            // Clone the input histogram to output histogram.
            TH1F* outputHistogram = (TH1F*)inputHistogram->Clone();

            // Write the histogram to the output file.
            outputHistogram->Write();

            for (Int_t i = 1; i <= outputHistogram->GetNbinsX(); i++) {
                TH1D* projX = outputHistogram->ProjectionX("proj", i, i);
                TH1D* projY = outputHistogram->ProjectionY("proj", i, i);
                for (Int_t j = 1; j <= projX->GetNbinsX(); j++) {
                    h2->SetBinContent(i, j, projX->GetBinContent(j));
                }
    ...
    // Name of the histogram in the ROOT files
    string histName = "myHist";

    // Vector to hold all histograms
  ...
  // --------------- Output File ------------------
    ...
    TFile* myFile = new TFile("DeltaY_UL18_muon_750_900_ttbar_all.root", "RECREATE");
